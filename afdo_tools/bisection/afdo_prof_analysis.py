...
def text_to_json(f: IO[str]) -> dict[str, str]:
    ...
    generated by bin/llvm-profdata (within an LLVM build).
    """
    results = {}
    curr_func = None
    curr_data = []
...
class DeciderState:
    ...
    def __init__(
        ...
        self.accumulated_results: list[int] = []
        self.external_decider = external_decider
        # imported from a previous run of this script
        self.saved_results: list[int] = []
    ...
    def run(self, prof: dict[str, str], save_run: bool = True) -> StatusEnum:
        ...
        if return_code in statuses:
            ...
            if status == StatusEnum.PROBLEM_STATUS:
                if self.save_problem_decider:
                    prof_file = prof_to_tmp(prof)
                    raise RuntimeError(
                        "Provided decider script returned PROBLEM_STATUS "
                        f"when run on profile stored at {prof_file}. AFDO "
                        "Profile analysis aborting"
                    )
                raise RuntimeError(
                    "Provided decider script returned PROBLEM_STATUS "
                    "when run on profile. AFDO Profile analysis aborting"
                )
            if save_run:
...
def bisect_profiles(
    ...
    Args:
        ...
        common_funcs: the list of functions which have top-level profiles in
          both 'good' and 'bad'
    ...
    'ranges': a list of lists of function names. Each list of functions is a
      list such that including all of those from the bad profile makes the good
    ...
    results: dict[str, Any] = {"individuals": [], "ranges": []}
    if hi - lo <= 1:
        logging.info(
...
def range_search(
    ...
    lo_mid_funcs: list[str] = []
    mid_hi_funcs: list[str] = []
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no_save_problem_decider",
        action="store_false",
        dest="save_problem_decider",
        help="""
        If the 'external_decider' returns a problem status, this script usually
        saves the problematic AFDO file in a file for postmortem debugging.
        Passing this disables that behavior.
        """,
    )
    parser.add_argument(
...
def main_impl(flags: argparse.Namespace) -> dict[str, Any]:
    ...
    decider = DeciderState(
        flags.state_file,
        flags.external_decider,
        seed=flags.seed,
        save_problem_decider=flags.save_problem_decider,
    )
